...
} from "../src/StorageProofVerifier.sol";
import {SparseMerkleTree, TreeEntry} from "../src/SparseMerkleTree.sol";

/// @dev Minimal Diamond stub: `storedBatchHash` configurable per batch.
contract MockZkSyncDiamond is IZkSyncDiamond {
    mapping(uint256 => bytes32) private _hashes;

    function setStoredBatchHash(uint256 batchNumber, bytes32 h) external {
        _hashes[batchNumber] = h;
    }

    function storedBatchHash(uint256 batchNumber) external view returns (bytes32) {
        return _hashes[batchNumber];
    }
}

contract StorageProofVerifierTest is Test {
    SparseMerkleTree internal smt;
    MockZkSyncDiamond internal diamond;
    StorageProofVerifier internal verifier;

    /// @dev Same vectors as `SparseMerkleTree.t.sol::test_getRootHash`
    bytes32 internal constant EXPECTED_ROOT =
        0xcffaa7db2e75d764007ded235fb8f482bc8a43ce14e35a4f1a979c485f3c7fc6;

    function setUp() public {
        smt = new SparseMerkleTree();
        diamond = new MockZkSyncDiamond();
        verifier = new StorageProofVerifier(diamond, smt);
    }

    function _sampleProof() internal pure returns (StorageProof memory p) {
        ...
        p = StorageProof({
            metadata: BatchMetadata({
                batchNumber: 4242,
                indexRepeatedStorageChanges: 2,
                numberOfLayer1Txs: 3,
                priorityOperationsHash: bytes32(uint256(0x04)),
                dependencyRootsRollingHash: bytes32(uint256(0x05)),
                l2LogsTreeRoot: bytes32(uint256(0x06)),
                timestamp: 7,
                commitment: bytes32(uint256(0x08))
            }),
            account: account,
            key: entry.key,
            value: entry.value,
    ...
    function test_verify_acceptsV1StoredBatchHashEncoding() public {
        StorageProof memory p = _sampleProof();
        StoredBatchInfo memory batchV1 = StoredBatchInfo({
            batchNumber: p.metadata.batchNumber,
            batchHash: EXPECTED_ROOT,
            indexRepeatedStorageChanges: p.metadata.indexRepeatedStorageChanges,
            numberOfLayer1Txs: p.metadata.numberOfLayer1Txs,
            priorityOperationsHash: p.metadata.priorityOperationsHash,
            dependencyRootsRollingHash: p.metadata.dependencyRootsRollingHash,
            l2LogsTreeRoot: p.metadata.l2LogsTreeRoot,
            timestamp: p.metadata.timestamp,
            commitment: p.metadata.commitment
        });
        bytes32 expected = keccak256(abi.encode(batchV1));
        diamond.setStoredBatchHash(p.metadata.batchNumber, expected);
        assertTrue(verifier.verify(p), "v1 hash should verify");
    }

    function test_verify_acceptsLegacyStoredBatchHashEncoding() public {
        StorageProof memory p = _sampleProof();
        ...
        bytes32 expected = keccak256(abi.encode(batchLegacy));
        diamond.setStoredBatchHash(p.metadata.batchNumber, expected);
        assertTrue(verifier.verify(p), "legacy hash should verify");
    }

    function test_verify_rejectsWrongStoredBatchHash() public {
        StorageProof memory p = _sampleProof();
        diamond.setStoredBatchHash(p.metadata.batchNumber, bytes32(uint256(1)));
        assertFalse(verifier.verify(p));
    }

    function test_verify_rejectsWrongDependencyRootsForV1Chain() public {
        StorageProof memory p = _sampleProof();
        StoredBatchInfo memory batchV1 = StoredBatchInfo({
            batchNumber: p.metadata.batchNumber,
            batchHash: EXPECTED_ROOT,
        ...
        p.metadata.dependencyRootsRollingHash = bytes32(uint256(0xdead));
        assertFalse(verifier.verify(p));
    }
}
